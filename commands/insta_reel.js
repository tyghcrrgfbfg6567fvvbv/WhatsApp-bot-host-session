 ...
 * description: Download an Instagram Reel by URL and send it to the user
 * example: .insta_reel https://www.instagram.com/reel/C1234567890/
 * subcommands:
 *   - cmd: <URL>
 *     desc: Provide the Instagram Reels URL
 */
...
module.exports = {
  name: "insta_reel",
  description: "Download and send Instagram Reels to the requesting WhatsApp user",
    ...
    try {
      ...
      if (args.length < 2) {
        await XeonBotInc.sendMessage(sender, {
          text: "❌ Please provide an Instagram Reels URL (Example: .insta_reel https://www.instagram.com/reel/VIDEO_ID/)",
        });
        return;
      }
      ...
      if (!url.includes("instagram.com")) {
        await XeonBotInc.sendMessage(sender, {
          text: "❌ Invalid Instagram URL. Please provide a valid Reels URL.",
        });
        return;
      }
      ...
      const tempFile = path.join(VIDEO_DIR, `temp_reel_${Date.now()}.mp4`);

      // Log download start
      console.log(
        `📥 INCOMING [${sender}]: Starting Instagram Reels download for URL "${url}"`
      );

      // Download with yt-dlp, re-encoding to H.264/AAC
      const command = `yt-dlp -f "best[ext=mp4]" --recode-video mp4 -o "${tempFile}" "${url}" --postprocessor-args "-c:v libx264 -c:a aac"`;
      ...
      console.log(
        `📤 OUTGOING [${sender}]: Instagram Reels uploaded for URL "${url}"`
      );
      ...
      console.error(
        `Error in insta_reel command: ${error.message}`,
        error.stack
      );
      await XeonBotInc.sendMessage(msg.key.remoteJid, {
        text: `❌ An error occurred while downloading or sending the Reel: ${error.message}`,
      });
      console.log(
        `📥 INCOMING [${msg.key.remoteJid}]: ❌ An error occurred while downloading or sending the Reel.`
      );
    }
  },
};
